...
io.on("connection", (socket) => {
	...
	socket.on("lossData", (data) => {
		...
		savedData.push(data);
		console.log(`savedData size is ${savedData.length}`);
	});

	socket.on("check saved data", () => {
		console.log("save button clicked ->", savedData);
	});

	socket.on("replay", () => {
		//console.log("Btn Clicked here's lossData ->", savedData);
		savedData.forEach((el, index) => {
			setTimeout(() => {
				io.sockets.emit("sentLossData", el);
				console.log("now sending ->", el);
			}),
				2.0 * 1000;
		});
	});
